//LIENZO DEL CANVAS
const canvas = document.getElementById("lineaChart");
const ctx = canvas.getContext("2d");

//DATOS QUE SE VAN A GRAFICAR
const labels = ["Ene","Feb","Mar","Abr","May","Jun","Jul","AGO","Sep","Oct","Nov","Dic"];
const sanSalvador = [30, 32, 34, 35, 36, 37, 35, 34, 32, 31, 30];
...
function drawLineWithLabels(data,color){
    ...
    for(let i=0; i<data.length; i++){
        ...
        ctx.fillText(data[i]+"°C", x+5, y-5); //(VALOR A MOSTRAR EN ETIQUETAS, POSICION EN EJE "X", POSICION EN EJE "Y"
    }
    ctx.stroke();//DIBUJAR
}
...
function drawAxes(){
    ...
    //EJE X
    ctx.moveTo(50,canvas.height-50); //UBICAR LA POSICION DEL PUNTERO RN EJE "X" e "Y"
    ...
    for (let i=0; i< labels.length; i++) {//SE RECORRE EL ARREGLO DE ETIQUETAS
         const x = (i/(labels.length - 1))*(canvas.width - 100) + 50; //LA ETIQUETA ESTARA EN LA MISMA POSICION EL PUNTERO
         ctx.fillText(labels[i],x,canvas.height - 30); //MOSTRAR EL TEXTO DEL ARREGLO "labels" EN LA POSICION CALCULADA EN X PERO 30px SUPERIOR        
    }

    //ETIQUETAS EN EJE Y
    for(let i=20; i<=40; i+=5){ //EN EL EJE Y SE MOSTRARAN RANGOS DESDE LOS 20°C HASTA LOS 40°C CON SALTOS DE 5°C
        const y = canvas.height - 50 - (i-20) * 10; //POSICION DE CADA GRADO CENTRIGRADO, IRA DECREMENTANDO
...
ctx.fillStyle = 'red';
ctx.fillRect(70,20,10,10); //DIBUJAR UN RECTANGULO DE 70px DE ANCHO, 20px DE ALTO, POSIXION X=10, POSICION Y=10
ctx.fillStyle = 'black';
ctx.fillText("San Salvador",85,30); //COLOCAR EL TEXTO:"SAN SALVADOR" EN LA POSICION EJE X=85, EJE Y=30

ctx.fillStyle = 'blue';
ctx.fillRect(170,20,10,10); //DIBUJAR UN RECTANGULO DE 170px DE ANCHO, 20px DE ALTO , POSICION X=10, Y =10
ctx.fillStyle = 'black';
